...
#define FFT_DATA_MAXIMUM_VALUE 2000000//1000 //40000 //4086 //14000	//80000		//Represents the max value that is possible after FFT the data.
#define DC_CUT_OFFSET 180 //12 chnaged to zero bc photoreceiver removes envelop
#define UPPER_NORMALIZATION_THRESHOLD 1.0f//1.0f
#define LOWER_NORMALIZATION_THRESHOLD 0.0f//0.7f
...
float* h_test1 = (float*)malloc(floatmem_size);
...
// NORMALIZATION KERNEL FUNCTION:
__global__ void d_normalization(float* normalize_result, float4* result, int numElements, float normalize_max, float* testdata) {
    int i = blockDim.x * blockIdx.x + threadIdx.x;

    if (i < numElements) {
        result[i] = make_float4(0.7f, 0.498039f, 0.196078f, (((normalize_result[i]) / (normalize_max)) * 1.0f));
        //result[i] = make_float4(0.7f, 0.498039f, 0.196078f, normalize_result[i]/10000.0 * 1.0f);

        if (result[i].w > UPPER_NORMALIZATION_THRESHOLD) {
            result[i].w = UPPER_NORMALIZATION_THRESHOLD;
        }
        else if (result[i].w <= LOWER_NORMALIZATION_THRESHOLD) {
            result[i].w = 0.0f;
        }

        result[i].w = (result[i].w / UPPER_NORMALIZATION_THRESHOLD);
        //testdata[i] = (result[i].w / UPPER_NORMALIZATION_THRESHOLD);
        //result[i].w = result[i].w ;
        //testdata[i] = result[i].w ;
    }
}
...
__global__ void d_magnitude(cufftComplex* data, float4* result, int numElements, float* testdata, float* normalize_result) {
...
void runCuda(struct cudaGraphicsResource** vbo_resource)
{
    ...
    if (determinedPeriod > 1000) // i.e. determinedPeriod = 2000
    d_magnitude <<< (XLENGTH * ZLENGTH), YLENGTH >>> (d_data, dptr, (int) (DATASIZE/4), d_test, d_normalize);
    else
    d_magnitude <<< (XLENGTH * ZLENGTH), YLENGTH >>> (d_data, dptr, DATASIZE, d_test, d_normalize);

    cudaMemcpy(h_test, d_normalize, floatmem_size, cudaMemcpyDeviceToHost);
...
bool initGL(int* argc, char** argv)
{
    ...
    glutCreateWindow("Swept Source OCT (*PRESS ESC FOR PROPER CLOSE*)");
    glutDisplayFunc(display);
    //glutKeyboardFunc(keyboard);
    //glutMotionFunc(motion);
    ...
    glLoadIdentity();
    gluPerspective(60.0, (GLfloat)window_width / (GLfloat)window_height, 0.0, 1000.0); //Determines the clipping range for how much you can see.
